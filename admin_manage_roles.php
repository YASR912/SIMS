...
// ✅ FIX: Try all common session key names for user ID
...
if (!$adminId) {
    die("Session error: Could not find admin user ID in session. Keys available: " . implode(', ', array_keys($_SESSION)));
}
$adminId = intval($adminId);

// Safe helper — logs action without crashing if FK fails
function logAction($con, $userId, $action, $adminId) {
    try {
        $logQuery = "INSERT INTO activity_log (user_id, action, performed_by) VALUES (?, ?, ?)";
        $logStmt = $con->prepare($logQuery);
        $logStmt->bind_param('isi', $userId, $action, $adminId);
        $logStmt->execute();
    } catch (Exception $e) {
        // Log silently failed — don't crash the page
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'promote') {
        ...
        if ($stmt->execute()) {
            logAction($con, $userId, 'Promoted to Manager', $adminId);
        ...
        $stmt->bind_param('ii', $adminId, $userId);
        if ($stmt->execute()) {
            logAction($con, $userId, 'Demoted to Employee', $adminId);
        ...
        // ✅ Log BEFORE delete — FK requires user to still exist
        logAction($con, $userId, 'User Removed', $adminId);
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        while ($log = mysqli_fetch_assoc($logResult)): ?>
                                            <tr>
                                                <td><?php echo htmlspecialchars($log['Username'] ?? 'Removed User'); ?></td>
